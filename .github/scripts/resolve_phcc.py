...
# Bound to a name rather than written inline: at target-version "py314" ruff's
# formatter drops the parentheses from an inline `except (A, B):`, which
# parses only from Python 3.14 on (PEP 758) - the exact syntax error this
# tuple exists to avoid. A name in an `except` clause is never reformatted.
_UNUSABLE_PIN_ERRORS = (InvalidRequirement, InvalidVersion)
...
def newest_stable(pins: dict[str, str | None]) -> str:
    ...
    for version in sorted(pins, key=Version, reverse=True):
...
def resolve_from_pypi(candidates: int = 20) -> str:
    ...
    recent = sorted(published, key=Version, reverse=True)[:candidates]
    pins = {
