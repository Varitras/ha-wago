...
A test that passes is not evidence. A test that passes *and* fails when the
code it guards is broken is. A test that establishes the condition the
production code is supposed to establish - building the replacement object
itself, setting the flag under test, asserting that a step ran rather than
what it produced - passes either way and guards nothing. Reading such a test
rarely reveals that; breaking the code underneath it always does.
...
interrupted run say nothing about the mutation, so reading any non-zero exit
as success would make this script report what it wants to hear.
